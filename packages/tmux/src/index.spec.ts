import { SessionId, generateTmuxStartCommand, isInTmux, getCurrentSessionId } from "./index";
...
const POST_CMDS = [
    ";", "new-window", "-n", "daemon", "/tmp/node /tmp/jug.js daemon",
    ";", "new-window", "-n", "target", "/tmp/node /tmp/jug.js target",
    ";", "select-window", "-t:0", 
    ";", "attach"
];
...
test("generateTmuxStartCommand creates a command for a single pane", () => {
    ...
    expect(generateTmuxStartCommand(id, layout, TEST_JUG)).toEqual({
        command: "tmux",
        args: ["new-session", "-d", "-n", "test", "-s", id, "command", ...POST_CMDS],
    });
});
...
test("generateTmuxStartCommand creates a command for multiple panes", () => {
    ...
    expect(generateTmuxStartCommand(id, layout, TEST_JUG)).toEqual({
        command: "tmux",
        args: ["new-session", "-d", "-n", "test2", "-s", id, "command", ";",
            "split-window", "/tmp/node /tmp/jug.js component foobar", ";",
            "split-window", "conch", ...POST_CMDS],
    });
});
...
test("generateTmuxStartCommand creates a command for multiple windows", () => {
    ...
    expect(generateTmuxStartCommand(id, layout, TEST_JUG)).toEqual({
        command: "tmux",
        args: ["new-session", "-d", "-n", "test3", "-s", id, "command", ";",
            "split-window", "/tmp/node /tmp/jug.js component foobar", ";",
            "split-window", "conch", ";",
            "new-window", "-n", "test4", "echo 'foo'", ";",
            "split-window", "conch", ...POST_CMDS],
    });
});
...
test("isInTmux correctly checks the TMUX var", () => {
    const execSync = jest.fn()
        ...
        .mockReturnValueOnce("/dev/pty/test1 notCorrect1\n/dev/pty/test2 notCorrect2")
        .mockReturnValueOnce("/dev/pty/test3 correct1\n/dev/pty/test4 notCorrect3");
